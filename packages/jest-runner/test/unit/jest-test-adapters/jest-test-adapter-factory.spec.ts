...
describe(jestTestAdapterFactory.name, () => {
  ...
  it('should throw an error when the Jest version is lower than 22.0.0', () => {
    ...
    expect(act).to.throw(
      Error,
      'You need Jest version >= 22.0.0 to use the @d-es-ign/stryker-js-jest-runner plugin, found 21.0.0',
    );
  });
  ...
  it('should throw an error when the Jest version is an alpha version lower than 22.0.0', () => {
    ...
    expect(act).to.throw(
      Error,
      'You need Jest version >= 22.0.0 to use the @d-es-ign/stryker-js-jest-runner plugin, found 21.0.0-alpha.6',
    );
  });
  ...
  it('should throw an error when the Jest version is between 22 and 24, but coverage analysis is enabled', () => {
    ...
    expect(act).to.throw(
      Error,
      'You need Jest version >= 24.0.0 to use the @d-es-ign/stryker-js-jest-runner with "coverageAnalysis": "all", you\'re currently using version 23.0.0. Please upgrade your jest version, or set "coverageAnalysis": "off".',
    );
  });
